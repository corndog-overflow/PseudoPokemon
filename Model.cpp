...
Model :: Model()
{
    ...
    WildPokemon* W1 = new WildPokemon("Pikachu",1,2,1,1,Point2D(10,12));
    WildPokemon* W2 = new WildPokemon("Charmander",1,10,1,2,Point2D(15,5));
...
bool Model :: Update()
{
    ...
    list<WildPokemon*> :: iterator iter4;
    for(iter4 = wild_ptrs.begin(); iter4 != wild_ptrs.end(); iter4++)
    {
        if((*iter4)->ShouldBeVisible())
        {
            // Try to follow trainers
            for(list<Trainer*> :: iterator iter5 = trainer_ptrs.begin(); iter5 != trainer_ptrs.end(); iter5++)
            {
                if ((*iter5)->ShouldBeVisible())
                {
                    (*iter4)->follow(*iter5);
                }
            }
        } else if ((*iter4)->get_in_combat())
        {
            (*iter4)->get_attack();
        }
    }
...
void Model :: Display(View& view)
{
    ...
    {
        if (dynamic_cast<WildPokemon*>(*iter1) == nullptr)
        {
            view.Plot(*iter1);
        } else if (dynamic_cast<WildPokemon*>(*iter1)->get_in_combat() == false)
        {
            view.Plot(*iter1);
        }
    }
...
void Model :: NewCommand(char type, int in_id, Point2D p)
{
    switch(type)
    {
        case 'c':
        {
            ...
            PokemonCenter* C = new PokemonCenter(in_id, 100, 2, Point2D(p.x,p.y));
        ...
        case 'g':
        {
            ...
            PokemonGym* G = new PokemonGym(25,2,10,5,in_id,Point2D(p.x,p.y));
        ...
        case 't':
        {
            ...
            Trainer* T = new Trainer("NewTrainer",in_id,'S',1,Point2D(p.x,p.y));
        ...
        case 'w':
        {
            ...
            {
                if(in_id == (*iter1)->getId())
                    throw Invalid_Input("Sorry but please enter a Unique WildPokemon ID");
            }
            WildPokemon* W = new WildPokemon("neWild", 1, 2, 1, in_id, Point2D(p.x,p.y));
