...
void Trainer :: StartRecoveringHealth(unsigned int num_potions)
{
    ...
    else if(!(current_center->canAffordPotions(num_potions,PokeDollars)))
    {
...
void Trainer :: ShowStatus()
{
    ...
    cout << "Being followed by " << this->wild_pokemon_count << " Wild Pokemon" << endl;
}
...
void Trainer :: add_to_wild_pokemon_count()
{
    wild_pokemon_count++;
}

void Trainer :: remove_from_wild_pokemon_count()
{
    wild_pokemon_count--;
}

void Trainer ::ReduceTrainerHealth(double points)
{
    health -= points;
}
